...
static void set_duty_cycle(PhasePinConfig_t *config, uint32_t dutyValue,
                           bool enableOutput);
...
static void timer_init() {
  for (size_t i = 0; i < NUM_PHASES; i++) {
    ...
    HAL_GPIO_WritePin(config->OutputEnableGPIOPort, config->OutputEnableGPIONum,
                      false);
  }
}

void pwm_init() { timer_init(); }
